<script>
    ...
    const submitUpdateProfile = () => {
        ...
        return async({ result }) => {
            switch(result.type) {
                case 'success' : await invalidateAll()
                break

                case 'error' : break

                default: await applyAction(result)
            }
            loading = false;
        }
    }
...
<div class="flex flex-col w-full h-full">
    <form action="?/updateProfile" method="POST" class="flex flex-col space-y-2 w-full" enctype="multipart/form-data" use:enhance={submitUpdateProfile}>
        ...
        <div class="w-full-max-w-lg form-control">
            ...
            <label for="avatar" class="avatar w-32 rounded-full hover:cursor-pointer">
                ...
                <div class="w-32 rounded-full">
                    ...
                    `https://api.dicebear.com/5.x/thumbs/svg?size=32&seed=${data.user?.name}&scale=70`} alt="User Profile Avatar" id="avatar-preview" />
                </div>
            </label>
        ...
        <Input id="name" label="Name" value={data?.user?.name}  disabled={loading} errors={form?.errors?.name}/>
